#!/bin/bash
# F9 runner v5: surgical success_prob patch; ENTANGLED-only latency; unbuffered.
set -e
cd ~/fpqr/fpqr-engine
mkdir -p tools
...
def install_swap_patch():
    ...
    def patched(self, *args, **kwargs):
        ...
        # surgical: only the probability fields; is_success is a state flag
        for a in ("success_prob", "success_probability"):
            if hasattr(self, a):
                try:
...
# ---------------------------------------------------------------------------
def main():
    print("SeQUeNCe F9 probe v5: first-delivery latency with forced p_swap")
    print("=" * 68)
    install_swap_patch()
...
if __name__ == "__main__":
    main()
PYEOF
echo "=== probe written: $(wc -c < tools/seq_f9_probe.py) bytes (v5) ==="
