#!/bin/bash
# F9 runner v8: fidelity 0.35 + ground-truth memory fidelity/coherence diagnostic.
set -e
cd ~/fpqr/fpqr-engine
mkdir -p tools
...
# ---------------------------------------------------------------------------
def measure(n_routers, p_swap, n_seeds=3, fidelity=0.35):
...
# ---------------------------------------------------------------------------
def main():
    print("SeQUeNCe F9 probe v8: first-delivery latency with forced p_swap")
    print("=" * 68)
    install_swap_patch()
...
if __name__ == "__main__":
    main()
PYEOF
echo "=== probe written: $(wc -c < tools/seq_f9_probe.py) bytes (v8) ==="
