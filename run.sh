#!/bin/bash
# F9 runner: self-contained, venv-self-detecting.
# Update THIS ONE FILE in the GitHub web editor each round.
set -e
cd ~/fpqr/fpqr-engine
mkdir -p tools
...
def install_swap_patch():
    ...
    def patched(self, *args, **kwargs):
        ...
        # set every success-related data attribute to our p, plus the known name
        for a in dir(self):
            if "success" in a.lower():
                try:
                    if isinstance(getattr(self, a), (int, float)):
                        setattr(self, a, _SWAP_STATE["p"])
                except Exception:
                    pass
        try:
...
# ---------------------------------------------------------------------------
def measure(n_routers, p_swap, n_seeds=5, fidelity=0.8):
    from sequence.topology.router_net_topo import RouterNetTopo
    from sequence.app.request_app import RequestApp

    _SWAP_STATE["p"] = p_swap
    stop_time_ps = int(2e13)          # 20 s cap
    start_t = int(1e12)               # request opens at 1 s
    lat, timeouts, swaps_before = [], 0, _SWAP_STATE["count"]

    for s in range(n_seeds):
        cfg = build_linear_config(n_routers, 1000.0, 0.0002, 50, s, stop_time_ps)
        ...
        if rec.get("first") is not None:
            lat.append((rec["first"] - start_t) * 1e-12)
        else:
            timeouts += 1
...
# ---------------------------------------------------------------------------
def main():
    print("SeQUeNCe F9 probe v2: first-delivery latency with forced p_swap")
    print("=" * 68)
    install_swap_patch()
    ...
    for label, n in cells:
        for p in (0.5, 1.0):
            ...
            try:
                ...
                print(f"    timeouts: {to}/5   swap instances created: {swaps}")
            except Exception:
                import traceback
                traceback.print_exc()
...
if __name__ == "__main__":
    main()
PYEOF
echo "=== probe written: $(wc -c < tools/seq_f9_probe.py) bytes ==="

# find a python that can import sequence (the venv), without needing activation
PYBIN=""
...
echo "=== using: $PYBIN ==="
"$PYBIN" tools/seq_f9_probe.py 2>&1 | tee /tmp/f9_round.txt
