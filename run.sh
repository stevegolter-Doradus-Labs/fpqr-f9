#!/bin/bash
# F9 runner v12: FULL cross-check, request fidelity 0.30 (per v11 diagnostic).
set -e
cd ~/fpqr/fpqr-engine
mkdir -p tools
cat > tools/seq_f9_runner.py << 'PYEOF'
#!/usr/bin/env python3
"""
...
Built from the machinery validated by probe rounds v1-v9:
  ...
  - request fidelity threshold 0.30 (v11 diagnostic: SeQUeNCe applies per-swap
    degradation, so H=12 final fidelity sits in [0.30, 0.50)); below every
    cell's final fidelity, so no purification and no fidelity gating
...
def _one_seed(task):
    ...
    # Request fidelity 0.30 uniformly: diagnostic v11 showed H=10..12 final
    # fidelity lands in [0.30, 0.50) because SeQUeNCe applies a per-swap
    # degradation factor on top of the combination formula. 0.30 is below
    # every subset cell's final fidelity and above the separable floor, so
    # no purification triggers and no cell is fidelity-gated; identical for
    # B0 and B3, so comparisons are internally consistent. Disclosed in 12.7.
    app.start(dst.name, start_t, stop_time_ps - int(1e12), 1, 0.30)
    tl.init(); tl.run()
...
# ---------------------------------------------------------------------------
class SeqBackend:
    name = "sequence-1.0.0 (calibrated)"
    def __init__(self, kappa):
        self.kappa = kappa
        self.raw_means = {}
    def simulate(self, cell, n_seeds, seed):
        from fpqr_sim.baselines import Design
        n_routers = (cell.H + 1) if cell.design == Design.B0 else (cell.d_bridge + 1)
        t0 = time.time()
        raw = seq_latency_samples(n_routers, cell.p_swap, cell.link_F,
                                  cell.tau, n_seeds, seed)
        good = np.isfinite(raw).sum()
        self.raw_means[cell.key()] = (float(np.nanmean(raw))
                                      if np.isfinite(raw).any() else float("nan"))
        rawm = float(np.nanmean(raw)) * 1e3 if np.isfinite(raw).any() else float("nan")
        print(f"  [seq] {cell.key():44s} {good}/{n_seeds} ok  "
              f"raw {rawm:8.4f} ms  wall {time.time()-t0:6.1f}s",
              flush=True)
        return raw * self.kappa

# ---------------------------------------------------------------------------
def main():
    print("F9 cross-check runner v12 (SeQUeNCe backend, threshold 0.30)", flush=True)
    print("=" * 76, flush=True)
    install_swap_patch()
    from fpqr_sim.driver import Cell, simulate_cell
    from fpqr_sim.baselines import Design
    from fpqr_sim.tier2_crosscheck import run_cross_check

    # ---- calibration on the reference cell (H=2, p_swap=1.0, B0) ----
    ref = Cell("T1", H=2, p_swap=1.0, tau=1.0, design=Design.B0)
    print("calibration reference:", ref.key(), flush=True)
    rng = np.random.default_rng(424242)
    t3_ref = simulate_cell(ref, 100, rng)["roam_latency_s"]
    t3_ref = t3_ref[np.isfinite(t3_ref)]
    s_ref = seq_latency_samples(3, 1.0, ref.link_F, ref.tau, 100, 424242)
    s_ref = s_ref[np.isfinite(s_ref)]
    kappa = float(t3_ref.mean() / s_ref.mean())
    print(f"  tier3 ref mean {t3_ref.mean():.6e} s | seq ref mean "
          f"{s_ref.mean():.6e} s | kappa {kappa:.6e}", flush=True)

    # ---- full validation subset ----
    backend = SeqBackend(kappa)
    res = run_cross_check(backend, n_seeds=100, fraction=0.05, seed=0)

    # ---- report ----
    print("\n" + "=" * 76)
    print(f"backend: {res.backend}   cells: {res.n_cells}")
    print(f"within 5% band: {res.frac_within_5pct:.0%}   "
          f"flagged (true divergence): {res.frac_flagged:.0%}   "
          f"max divergence: {res.max_divergence:.1%}")
    print(f"verdict: {res.verdict}")
    print("\nper-cell table (t3 = tier3 mean s, t2 = calibrated seq mean s):")
    print(f"{'cell':44s} {'t3':>12s} {'t2':>12s} {'div':>7s} {'CIovl':>5s} {'FLAG':>4s}")
    for a in res.agreements:
        print(f"{a.cell_key:44s} {a.tier3_mean_s:12.5e} {a.tier2_mean_s:12.5e} "
              f"{a.rel_divergence:7.1%} {str(a.ci_overlap):>5s} "
              f"{'YES' if a.flagged else '-':>4s}")

    from dataclasses import asdict
    out = asdict(res)
    out["kappa"] = kappa
    out["raw_seq_means"] = backend.raw_means
    path = os.path.expanduser("~/f9_results.json")
    with open(path, "w") as f:
        json.dump(out, f, indent=1)
    print(f"\nfull results written: {path}", flush=True)

if __name__ == "__main__":
    main()
PYEOF
echo "=== runner written: $(wc -c < tools/seq_f9_runner.py) bytes (v12) ==="
PYBIN=""
...
echo "=== using: $PYBIN ==="
PYTHONUNBUFFERED=1 "$PYBIN" -u tools/seq_f9_runner.py 2>&1 | tee /tmp/f9_full.txt
