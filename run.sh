#!/bin/bash
# F9 runner v3: fast probe (early stop at first delivery, progress per seed).
set -e
cd ~/fpqr/fpqr-engine
mkdir -p tools
...
def attach_latency_hook(app, timeline, record, diag_budget=3):
    ...
    def hooked(*args, **kwargs):
        ...
        if record.get("first") is None:
            record["first"] = t
            # stop simulating: we only need the first delivery
            try:
                timeline.stop()
            except Exception:
                try:
...
# ---------------------------------------------------------------------------
def measure(n_routers, p_swap, n_seeds=3, fidelity=0.8):
    from sequence.topology.router_net_topo import RouterNetTopo
    from sequence.app.request_app import RequestApp

    _SWAP_STATE["p"] = p_swap
    stop_time_ps = int(6e12)          # 6 s cap; early stop makes this a bound, not a cost
...
if __name__ == "__main__":
    main()
PYEOF
echo "=== probe written: $(wc -c < tools/seq_f9_probe.py) bytes (v3) ==="
