#!/bin/bash
# F9 runner v10: FULL 39-cell cross-check (SeQUeNCe backend, 20-core, calibrated).
set -e
cd ~/fpqr/fpqr-engine
mkdir -p tools
...
class SeqBackend:
    ...
    def simulate(self, cell, n_seeds, seed):
        ...
        print(f"  [seq] {cell.key():44s} {good}/{n_seeds} ok  "
              f"raw {np.nanmean(raw)*1e3:8.4f} ms  wall {time.time()-t0:6.1f}s",
              flush=True)
        return raw * self.kappa

# ---------------------------------------------------------------------------
def main():
    print("F9 cross-check runner (SeQUeNCe backend)", flush=True)
    print("=" * 76, flush=True)
    install_swap_patch()
...
if __name__ == "__main__":
    main()
PYEOF
echo "=== runner written: $(wc -c < tools/seq_f9_runner.py) bytes (v10) ==="
